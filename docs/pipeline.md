...
* **Mechanics**:
  ...
  * **Vulkan GPU Acceleration (`--v360_backend=vulkan`)**: Offloads Phase 1 dual-fisheye projection remapping onto GPU compute shaders via FFmpeg's native `v360_vulkan` filter (`format=yuv420p,hwupload,v360_vulkan=...,hwdownload,format=yuv420p` using `-init_hw_device vulkan=vk -filter_hw_device vk`). Fully compatible with NVIDIA, AMD Radeon, and Intel Arc GPUs. Supports both standard dual-fisheye unwarping and complex split-lens seam blending (`--blend_seams`). Automatically and gracefully falls back to CPU `v360` if the startup hardware probe (`probe_vulkan()`) fails or when Quality Mode 3 (Lanczos high-order sinc resampling) is selected. Phase 2 (Gyro & Optical Flow Stabilization) remains on CPU `v360` where dynamic `sendcmd` Euler streams are required.
