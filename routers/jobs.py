...
async def api_start_job(request: Request):
    ...
    if str(gp("stabilize", "0")) == "1" and not is_photo:
        ...
        pipeline_cmd.extend(["--telemetry_mode", safe_choice(gp("telemetry_mode"), ["smooth", "level", "lock", "zero"], PIPELINE_DEFAULTS["telemetry_mode"])])
        pipeline_cmd.extend(["--telemetry_fusion", safe_choice(gp("telemetry_fusion"), ["none", "mahony", "complementary", "ekf"], PIPELINE_DEFAULTS["telemetry_fusion"])])
        pipeline_cmd.extend(["--telemetry_fusion_gain", safe_number(gp("telemetry_fusion_gain"), PIPELINE_DEFAULTS["telemetry_fusion_gain"])])
        pipeline_cmd.extend(["--telemetry_source", safe_choice(gp("telemetry_source"), ["auto", "samsung", "gopro", "camm", "insta360", "gyroflow", "witmotion", "custom_csv"], PIPELINE_DEFAULTS["telemetry_source"])])
        pipeline_cmd.extend(["--telemetry_smoothing", safe_number(gp("telemetry_smoothing"), PIPELINE_DEFAULTS["telemetry_smoothing"])])
        pipeline_cmd.extend(["--telemetry_ref_frame", safe_number(gp("telemetry_ref_frame"), PIPELINE_DEFAULTS["telemetry_ref_frame"], cast_fn=int)])
        ...
        pipeline_cmd.extend(["--kopf_cube_face", safe_number(gp("kopf_cube_face"), PIPELINE_DEFAULTS["kopf_cube_face"], cast_fn=int)])
        ...
        pipeline_cmd.extend(["--cinematic_window", safe_number(gp("cinematic_window"), PIPELINE_DEFAULTS["cinematic_window"], cast_fn=int)])
        pipeline_cmd.extend(["--traveldir_mode", safe_choice(gp("traveldir_mode"), ["travel_direction", "target_lock", "damped_follow"], PIPELINE_DEFAULTS["traveldir_mode"])])
